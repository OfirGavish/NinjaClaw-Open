...
If you believe you've found a security issue in NinjaClaw-Open, please report it privately via [GitHub Security Advisories](https://github.com/OfirGavish/NinjaClaw-Open/security/advisories/new).

For urgent issues, email **ofir@mscloudninja.com**.
...
3. Impact assessment
4. Affected component (file/function)
5. Suggested remediation

## Security Architecture

NinjaClaw-Open inherits OpenClaw's security model:

- **Container Isolation** — Agent code runs in sandboxed containers with read-only project mounts
- **Credential Injection** — Secrets are injected via environment, never written to disk
- **Allowlist Model** — Only approved senders can interact with the gateway
- **Channel Authentication** — Each messaging channel authenticates independently

## Trust Boundaries

- The **gateway operator** is trusted (controls config, credentials, and channel setup)
- **Allowlisted senders** are trusted within their permission scope
- **Non-allowlisted senders** cannot interact with agents
- **Container sandboxes** isolate agent execution from the host

## Out of Scope

- Prompt injection without a boundary bypass
- Operator-intended local features (shell access, canvas eval)
- Issues requiring adversarial operators on a shared host
- Findings from non-current versions

## Upstream

This project is based on [OpenClaw](https://github.com/openclaw/openclaw). For issues in the upstream codebase, report to the [OpenClaw security policy](https://github.com/openclaw/openclaw/security/policy).
